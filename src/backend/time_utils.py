...
# Functions
def date_time(time_string: str) -> pd.Timestamp:
    
    """
    Converts a string representing a date or time to a pandas Timestamp object.
    ...
    for date_format in ['%d-%m-%Y', '%d.%m.%Y', '%d/%m/%Y', '%Y-%m-%d', '%Y.%m.%d', '%Y/%m/%d', '%Y-%m-%d %H:%M:%S.%f', '%Y-%m-%d %H:%M:%S', '%Y-%m-%d %H:%M', '%d.%m.%Y %H:%M']:
        ...
        try:
            ...
            pass
            # print(f'Could not convert {time_string} to datetime because we are using date_format: {date_format}')
...
def validate_dateformat(df: pd.DataFrame, date_feature: str, date_str: str, date_formats: list[str]) -> bool:
    ...
    df[f"{date_feature}_dt"] = df[date_feature].apply(lambda x: date_time(x))
    date_str_dt = date_time(date_str)
...
def time_filter(df: pd.DataFrame, start_date: str, end_date: str, date_feature: str) -> pd.DataFrame:
    ...
    # Convert "date_str" and "date_feature" to datetime
    df[f"{date_feature}_dt"] = df[date_feature].apply(lambda x: date_time(x))
    start_date_dt = date_time(start_date)
    end_date_dt = date_time(end_date)
